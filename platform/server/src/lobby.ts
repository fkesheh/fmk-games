...
export class Lobby {
    ...
    try {
      ...
      switch (msg.t) {
        case 'list_rooms':
          ...
          break;
        case 'quick_join':
          this.quickJoin(sess, msg.name, msg.game);
          break;
        case 'create_public':
          this.createPublic(sess, msg.name, msg.game, msg.settings);
          break;
        case 'create_private':
          this.createPrivate(sess, msg.name, msg.game, msg.settings);
          break;
        case 'join_private':
  ...
  private quickJoin(sess: Session, name: string, game: string | undefined): void {
  ...
  private joinRoom(sess: Session, room: GameRoomHandle, name: string): void {
    const tracked = this.rooms.get(room.id);
    if (tracked !== undefined) tracked.emptySince = null;
    this.sessionRoom.set(sess.id, room);
    room.addPlayer(sess.id, name); // the room sends its own join payload
  }
