...
// fps-server file that imports @platform/shared). Owns the clientDist probe
...
function resolvePortDist_fps(): string {
  ...
  for (const dir of candidates) {
    if (_es(_p.join(dir, 'index.html'))) return dir;
  }
  const dev = candidates[0];
  if (dev === undefined) throw new Error('unreachable: empty candidate list');
  return dev;
...
export const fpsSdkModule = variantOf(fpsModule, {
  ...
  // P2P (docs/PLATFORM.md §12.6): fps-sdk rooms are rendezvous shells; the
  // match itself runs in the host player's tab (games/fps-sdk/client/src/p2p.ts).
  p2pShell: true,
});
