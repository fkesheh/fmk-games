// ============================================================================
// ANCIENTS·SDK P2P TRANSPORT (docs/PLATFORM.md §12.6) — the canonical SDK
// connection: the game's OWN menu and join flow, with the authoritative MOBA
// sim running in the host player's tab. The server's role shrinks to
...
//     RiftRoom adopts the shell code → frames loopback.
...
//
// Rift-wire flavour (outpost template, NOT kart/bank): the rift Game/Net
// expose NO socket-injection seam (WireOpts carries gameId + onOpenExtra
// ONLY, and games/rift is frozen) — so this transport ships a same-origin
// WebSocket shim instead: `new WebSocket(<same-origin>/ws)` returns a fake
// socket routed to the local HostedLobby (host) or the host's DataChannel
// (guest), and the standard boot in main.ts runs unmodified on top of it.
// The rendezvous signalling socket is the REAL WebSocket, captured before
// the shim is installed.
//
// snapshotTag/joinTag are the room's own wire tags: every per-tick broadcast
// is `{t:'rift_snap', …}` and the join receipt is `{t:'rift_hello', …}`
// (games/rift/server/src/room.ts sendJoinPayloads/pushSnapshots) —
// HostedLobby rewrites the shell code into both and replays the last snap on
// attach. The match-start verb is client `{t:'rift_start'}` → server
// `{t:'rift_begin'}` after the 3s lobby countdown (room.ts tryStart/lock).
//
// Rendezvous shells come from this port's own registry module (the
// 'ancients' riftModuleVariant with p2pShell:true): private shells are
// unlisted and join is code-scoped. The shell only carries presence
// (rtc_peers), the code (p2p_ready) and the rtc_signal hop. The actual
// game (RiftRoom sim + rift_* wire) runs entirely in the host tab.
//
// Bare @platform/sdk + @rift/shared specifiers (not the outpost template's
// relative paths): this client's tsconfig/vite config map both, and those
// config files are outside this task's scope. The RiftRoom import stays
// relative for the same reason the template's is: no config maps
// @rift/server, and relative paths resolve for tsc AND the vite build
// untouched.
// ============================================================================
import { loadIdentity, rng } from '@platform/shared';
import type { PlayerId } from '@platform/shared';
import { parseRiftSettings } from '@rift/shared';
import type { RiftSettings } from '@rift/shared';
import { RiftRoom } from '../../../rift/server/src/room.js';
import { HostedLobby } from '@platform/sdk/hosted.js';
import { Profiles } from '@platform/sdk/profile.js';
import { RtcStar } from '@platform/sdk/rtc.js';
import type { PcLike, SigChannel } from '@platform/sdk/rtc.js';
import { reportStats } from '@platform/sdk/stats.js';

/** Registry id of this port — the game id stats attribute to. */
const PORT_GAME_ID = 'ancients';
/** Rendezvous shell module (p2pShell: the ancients registry variant). */
const SHELL_GAME_ID = 'ancients';

/** Id-space draws for the host-tab room (room id + private code). The sim
 *  itself never touches this stream: bot brains seed from
 *  hashSeed(roomId, index) inside room.ts, exactly like server rooms. */
const rand: () => number = rng(((Date.now() ^ Math.floor(performance.now() * 0x9e3779b9)) >>> 0) || 1);

/** The room's own settings validation, applied to the host's create frame:
 *  garbage can never reach the RiftRoom constructor (a throw means the
 *  module's defaults — teamSize auto, speed 1). */
function resolveSettings(raw: unknown): RiftSettings {
  try {
    return parseRiftSettings(raw);
  } catch {
    return {};
  }
}
...
// ---- game-socket shim ---------------------------------------------------------
//
// Speaks the exact surface net.ts connect/send use (net.ts createNet):
...
export async function startP2p(): Promise<void> {
  ...
  // Join retry budget: the guest's join frame can lose a race with the host's
  // attach (DC open fires on both ends ~simultaneously; the host wires its
  // onMessage up to 100ms later and a frame sent into that window drops in
  // silence). rift_hello is the observable seat receipt, so the pump keeps
  // (re)sending the join until mySeat is set — every 5th tick, cheap.
  let rejoinCooldown = 0;
  let reconnectTimer: ReturnType<typeof setTimeout> | null = null;
  // Self-report identity, snooped off the local hello (stats need our seat).
  ...
  // Self-reported P2P stats: every client sees full snapshots plus the
  // broadcast rift_end, so each reports its OWN end-of-match counters from
  // its own seat row — same key taxonomy the server variant credits
  // (ancients.kill / ancients.death / ancients.win). One POST per distinct
  // end; best-effort (reportStats never throws into game code).
  function watchStats(json: string): void {
    const m = parseFrame(json);
    if (m === null) return;
    if (m.t === 'rift_hello' && typeof m.you === 'string') {
      mySeat = m.you;
      myTeam = m.team === 0 || m.team === 1 ? m.team : null;
      return;
    }
    if (authToken === null || m.t !== 'event') return;
    const ev = m.ev;
    if (typeof ev !== 'object' || ev === null) return;
    const e = ev as Record<string, unknown>;
    if (e.t !== 'rift_end' || !Array.isArray(e.stats)) return;
    const rows = e.stats.filter(
      (r): r is Record<string, unknown> => typeof r === 'object' && r !== null,
    );
    const code = typeof m.code === 'string' ? m.code : '';
    const key = `${code}:${rows.map((r) => JSON.stringify(r)).join('|')}`;
    if (key === lastEndKey) return;
    lastEndKey = key;
    const mine = rows.find((r) => r.id === mySeat);
    if (mine === undefined) return;
    const delta: Record<string, number> = {};
    if (typeof mine.kills === 'number') delta['ancients.kill'] = mine.kills;
    if (typeof mine.deaths === 'number') delta['ancients.death'] = mine.deaths;
    if ((e.winner === 0 || e.winner === 1) && e.winner === myTeam) delta['ancients.win'] = 1;
    if (Object.keys(delta).length === 0) return;
    const token: string | null = authToken;
    void reportStats(PORT_GAME_ID, delta, { getToken: () => token });
  }
  ...
  function onHostLinkLost(): void {
    ...
    guestWired = false;
    rejoinCooldown = 0;
    mySeat = null; // re-seat replays the join until the next hello
  ...
  function makeLobby(): HostedLobby {
    // Host-tab RiftRoom: private visibility (the shell code is the join
    // path), host-supplied settings through the room's own validator, the
    // module-scope id stream as deps.rand (server module.ts convention).
    return new HostedLobby({
      createRoom: (io, settings) => new RiftRoom('private', io, resolveSettings(settings), { rand }),
      newRoomCode: () => shell.code ?? 'LOCAL',
      snapshotTag: 'rift_snap',
      joinTag: 'rift_hello',
    });
  }
  ...
  function gameFrame(data: string): void {
    ...
    if (m.t === 'create_private' || m.t === 'create_public') {
      ...
      // The game's own `game` field never leaves the tab (rift Game stamps
      // 'rift' on creates); the shell verbs below carry the borrowed module.
      rendezvous.send(JSON.stringify({ t: m.t, name: displayName, game: SHELL_GAME_ID, settings: { p2p: true, ...settings } }));
    ...
    if (m.t === 'quick_join') {
      ...
      myJoinFrame = data;
      guestQueue.push(data);
      rendezvous.send(JSON.stringify({ t: 'quick_join', name: displayName, game: SHELL_GAME_ID }));
      say('finding a peer-to-peer table…');
      return;
    }
    if (m.t === 'join_public') {
      // Passthrough to the rendezvous socket (public SHELL rows join the
      // shell and continue over the DC). Caveat: this menu filters
      // RoomInfo.game === 'rift', so legacy server rows listed here would
      // seat the rendezvous session in a server room whose traffic this
      // transport ignores — private-code join is the supported P2P path.
      rendezvous.send(data);
      say('connecting to the host…');
      return;
    }
    if (m.t === 'ping') {
    ...
    // Room-level frames (rift_pick/rift_start/rift_order/…/leave/auth):
    // route by transport state. Pre-ready or unknown (incl. the shell's own
    // {t:'auth'} over the shim) they drop in silence — the room's parsers
    // would drop them too.
    if (lobby !== null && selfId === shell.hostId) {
  ...
  // Net's onopen-auth + queued frames flush through, exactly like a real
  ...
  const pump = setInterval(() => {
    ...
    if (!isHost && shell.hostId !== null) {
      ...
      if (cur !== null && cur !== undefined && shell.hostId !== null && shell.hostId !== selfId && myJoinFrame !== null && mySeat === null) {
        rejoinCooldown -= 1;
        if (rejoinCooldown <= 0) {
          rejoinCooldown = 5;
          clearReconnect();
  ...
  // window.__rift/__ancients surface. From here on Net's /ws sockets are
  // GameWs peers.
