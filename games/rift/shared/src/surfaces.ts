...
// kit's `surface(id)` / `emissiveSurface(id, colorKey, intensity)`. Nobody calls
// a material constructor directly, and nobody invents a family: adding a
// SurfaceId requires an orchestrator amendment.
//
...
export interface SurfaceEmissive {
   ...
   *  intensity)` overrides both fields with the caller's team/ability colour. */
...
export interface SurfaceDef {
  ...
  /** `material.transparent`. True for exactly one family — the river. Sorting
   *  transparent geometry is not free, and a transparent surface neither writes
   *  usable depth nor receives screen-space AO cleanly. */
...
export const SURFACES: Record<SurfaceId, SurfaceDef> = {
   ...
   *  `emissiveSurface('crystal', teamKey, intensity)`, which replaces both
   *  emissive fields, and must then be passed to `markBloom`. */
  crystal: {
    albedo: 'ward',
    roughness: 0.1,
    metalness: 0,
